...
public class MatrixPartitionModel {
    ...
    public static void main(String[] args) throws Exception {
        ...
        SingleOutputStreamOperator
                <GPSTrack> streamRow = env.addSource(new FlinkKafkaConsumer<>("sort1",
                new SimpleStringSchema(),properties).setStartFromEarliest()).map(new MapFunction<String, GPSTrack>() {
            @Override
         ...
         * OUT: 带有目标地址的点
         */
        SingleOutputStreamOperator<GPSTrack> streamColumn = env.addSource(new FlinkKafkaConsumer<>("sort2",
                new SimpleStringSchema(),properties).setStartFromEarliest()).map(new MapFunction<String, GPSTrack>() {
            @Override
            public GPSTrack map(String s) throws Exception {
                /**
...
//                        }
                ...
                }).writeAsText("/home/chenliang/out.txt", FileSystem.WriteMode.OVERWRITE).setParallelism(1);
